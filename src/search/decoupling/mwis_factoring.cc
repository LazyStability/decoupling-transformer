...
void MWISFactoring::PotentialLeaf::add_leaf_only_schema(
    ...
    if (find(action_schemes.begin(), action_schemes.end(), as_id) ==
        action_schemes.end()) {
...
MWISFactoring::MWISFactoring(const plugins::Options& opts)
    ...
    if (log.is_at_least_normal()) {
        ...
        case WMIS_STRATEGY::MCL:
            log << "maximize number of mobile conclusive leaves."
                << endl;
            break;
        ...
        log << "WARNING: This factoring strategy is PRELIMINARY..."
            << endl
            << "In most cases, it is superseded by the LP factoring "
               "that is based on CPLEX."
            << endl
            << "Please consider using the LP factoring for best "
...
void MWISFactoring::add_leaf_intersection_edges(
    Graph& graph, const vector<vector<size_t>>& var_to_p_leaves) const {
    // non-empty intersection between potential leaves
    vector<vector<bool>> pleaf_intersect(potential_leaf_nodes.size() -
                                         1);
    for (size_t i = 0; i < pleaf_intersect.size(); ++i) {
        pleaf_intersect[i].resize(potential_leaf_nodes.size() - i - 1,
                                  false);
    }
        ...
        for (size_t pot_leaf_1 : var_to_p_leaves[var]) {
            for (size_t pot_leaf_2 : var_to_p_leaves[var]) {
                if (pot_leaf_1 < pot_leaf_2) {
                    pleaf_intersect[pot_leaf_1]
                                   [pot_leaf_2 - pot_leaf_1 - 1] = true;
                }
            }
        }
    }
    for (size_t p_leaf_1 = 0; p_leaf_1 < potential_leaf_nodes.size();
         ++p_leaf_1) {
        for (size_t p_leaf_2 = p_leaf_1 + 1;
...
bool MWISFactoring::is_as_leaf_irrelevant(
    const ActionSchema& as, const PotentialLeaf& leaf) const {
...
bool MWISFactoring::is_as_leaf_conclusive(const ActionSchema& as,
                                          const PotentialLeaf& leaf) {
    // is every var in leaf contained in either the precondition of
    // effect of as
...
bool MWISFactoring::has_as_pre_or_eff_on_leaf(
    const ActionSchema& as, const PotentialLeaf& leaf) {
    for (int lvar : leaf.vars) {
        for (int pvar : as.pre_vars) {
            if (lvar == pvar) {
                return true;
...
void MWISFactoring::construct_graph_conclusive_leaves(
    Graph& /*graph*/) {
    // TODO implement this
    ...
    //        // mostly for trivially unsolvable task from
    //        translator? log << "ERROR: No action schemas." <<
    //        endl; return;
    //    }
    //
    //    compute_potential_leaves();
    //
    //    if (static_cast<int>(potential_leaves.size()) <
    //    min_number_leaves){
    //        log << "Only " << potential_leaves.size() <<
    //            " potential leaves left, but minimum number of
    //            leaves is " << min_number_leaves << "." << endl;
    //        return;
    //    }
    //
    //    if (!check_timeout()){
    ...
    //    }
    //
    //    vector<bool> can_be_leaf_var(task->get_num_variables(),
    //    false);
    //    // binary var for each potential leaf; 1 => becomes leaf,
    //    0 => not for (const auto &pleaf : potential_leaves) {
    //        for (int var : pleaf.vars){
    //            can_be_leaf_var[var] = true;
    //        }
    //        double obj = 0.0;
    //        if (strategy == WMIS_STRATEGY::MCL) {
    //            // for tie-breaking, give objective value of 1 to
    //            every mobile leaf obj = 1.0;
    //        }
    //        variables.push_back(lp::LPVariable(0.0, 1.0, obj,
    //        true));
    //    }
    //
    //    // an LP variable for every potential leaf that is 1 iff
    //    the leaf is conclusive vector<int> concl_pleaf_lp_vars; if
    //    (strategy == WMIS_STRATEGY::MCL) {
    //        assert(variables.size() ==
    //        static_cast<int>(potential_leaves.size())); for
    //        (size_t i = 0; i < potential_leaves.size(); ++i) {
    //            concl_pleaf_lp_vars.push_back(variables.size());
    //            variables.push_back(lp::LPVariable(0.0, 1.0,
    //            100.0, true));
    //        }
    //        assert(concl_pleaf_lp_vars.size() ==
    //        potential_leaves.size()); assert(variables.size() == 2
    //        * static_cast<int>(potential_leaves.size()));
    //    }
    //
    //    // binary var for each action schema of each potential
    //    leaf; 1 => is mobile, 0 => not vector<vector<size_t>>
    //    mob_as_var_ids(potential_leaves.size());
    //    // binary var for each action schema of each potential
    //    leaf; 1 => is conclusive for potential leaf, 0 => not
    //    vector<vector<size_t>>
    //    concl_as_var_ids(potential_leaves.size());
    //
    //    // for every potential leaf we store the list of action
    //    schemas relevant for the constraints;
    //    // for MCL, this is the schemas that can be neither
    //    irrelevant nor conclusive for the leaf,
    //    // for MCM, it's the schemas that become irrelevant or
    //    conclusive as global actions vector<vector<size_t>>
    //    relevant_as_by_pleaf(potential_leaves.size());
    //
    //    // list of LP variable IDs that represent mob_as_var_ids
    //    for every action schemas, i.e.,
    //    // the corresponding LP variable IDs across potential
    //    leaves vector<vector<size_t>>
    //    as_to_mob_as_var_ids(action_schemas.size());
    //
    ...
    //            variables.push_back(lp::LPVariable(0.0, 1.0, 0.0,
    //            true));
    //        }
    //        // for all action schemas as_id that are not part of
    //        pleaf, add a variable that represents if as_id is
    //        conclusive for pleaf for (size_t as_id = 0; as_id <
    //        action_schemas.size(); ++as_id) {
    //            if (find(pleaf.action_schemes.begin(),
    //            pleaf.action_schemes.end(), as_id) !=
    //            pleaf.action_schemes.end()){
    //                // action schema is in pleaf => cannot be
    //                conclusive for pleaf continue;
    //            }
    //            bool conclusive_or_irrelevant = false;
    //            if
    //            (has_as_pre_or_eff_on_leaf(action_schemas[as_id],
    //            pleaf)){
    //                if
    //                (is_as_leaf_conclusive(action_schemas[as_id],
    //                pleaf)){
    //                    conclusive_or_irrelevant = true;
    //                }
    //            } else if
    //            (is_as_leaf_irrelevant(action_schemas[as_id],
    //            pleaf)){
    //                conclusive_or_irrelevant = true;
    //            }
    //            if (strategy == WMIS_STRATEGY::MCM){
    //                if (!conclusive_or_irrelevant){
    //                    // the action schema cannot be conclusive
    //                    for this potential leaf, no need to add
    //                    variables for the objective value
    //                    continue;
    //                }
    //            } else {
    //                assert(strategy == WMIS_STRATEGY::MCL);
    //                if (conclusive_or_irrelevant){
    //                    // the action schema will be conclusive or
    //                    irrelevant for the leaf, no need to add a
    //                    constraint continue;
    //                }
    //            }
    //
    ...
    //        add_min_flexibility_constraints(constraints,
    //        mob_as_var_ids);
    //    }
    //
    //    if (min_mobility > 1){
    //        add_min_mobility_constraints(constraints,
    //        mob_as_var_ids);
    //    }
    //
    //    vector<size_t> c_vars_ids =
    //    add_center_variables_and_get_ids(variables, constraints,
    //    can_be_leaf_var);
    //
    ...
    //    }
    //
    //    add_potential_leaf_to_action_schema_constraints(constraints,
    //    mob_as_var_ids, can_be_leaf_var, c_vars_ids);
    //
    //    // add constraints such that conclusiveness of action
    //    schemas for pleaves is set properly for (const auto &pleaf
    //    : potential_leaves) {
    //        if (strategy == WMIS_STRATEGY::MCL) {
    //            lp::LPConstraint constraint1(-infty, 0.0);
    //            constraint1.insert(pleaf.id, -1.0);
    //            constraint1.insert(concl_pleaf_lp_vars[pleaf.id], 1.0);
    //            // pleaf can only be conclusive if it becomes a
    //            (mobile) leaf constraints.push_back(constraint1);
    //        }
    //        for (size_t i = 0; i <
    //        relevant_as_by_pleaf[pleaf.id].size(); ++i) {
    //            size_t as_id = relevant_as_by_pleaf[pleaf.id][i];
    //            if (strategy == WMIS_STRATEGY::MCM){
    //                lp::LPConstraint constraint1(-infty, 0.0);
    //                constraint1.insert(pleaf.id, -1.0);
    //                constraint1.insert(concl_as_var_ids[pleaf.id][i],
    //                1.0);
    //                // action schema can only be conclusive for
    //                pleaf if pleaf becomes a leaf
    //                constraints.push_back(constraint1);
    //
    //                // action schema can be conclusive for pleaf
    //                only if it is a global action schema, i.e.,
    //                iff it is not mobile for any potential leaf
    //                for (auto as_mob_var_id :
    //                as_to_mob_as_var_ids[as_id]) {
    //                    lp::LPConstraint constraint2(-infty, 1.0);
    //                    constraint2.insert(concl_as_var_ids[pleaf.id][i],
    //                    1.0);
    ...
    //                assert(strategy == WMIS_STRATEGY::MCL);
    //
    //                // action schema cannot be conclusive or
    //                irrelevant for pleaf so needs to be leaf-only
    //                for some potential leaf lp::LPConstraint
    //                constraint2(-infty, 0.0);
    //                constraint2.insert(concl_pleaf_lp_vars[pleaf.id],
    //                1.0); for (auto as_mob_var_id :
    //                as_to_mob_as_var_ids[as_id]) {
    //                    constraint2.insert(as_mob_var_id, -1.0);
    //                }
    //                constraints.push_back(constraint2);
...
void MWISFactoring::construct_graph(Graph& graph) {
    ...
    if (static_cast<int>(potential_leaf_nodes.size()) <
        min_number_leaves) {
...
vector<int>
MWISFactoring::solve_wmis(const Graph& graph,
                          const vector<double>& weights,
                          const utils::CountdownTimer& timer) {
    vector<int> independent_set;

    utils::g_log << "Computing max weighted independent set..."
                 << flush;
...
void MWISFactoring::compute_factoring_() {
    ...
    if (static_cast<int>(solution.size()) < min_number_leaves) {
        log << "WARNING: no factoring found with at least "
            << min_number_leaves << " leaves." << endl;
        return;
    }
...
inline double compute_leaf_fact_flexibility(
    const vector<int>& vars, const AbstractTask& task,
        ...
        for (int val = 0; val < dom_size; ++val) {
            ...
            for (size_t as_id : included_as) {
                if (facts_to_mobility[var][val].count(as_id) > 0) {
                    fact_mobility +=
                        facts_to_mobility[var][val].at(as_id);
                }
            }
            if (sum_fact_mobility[var][val] > 0) {
                // some facts might never be set by any action
                sum_fact_flexibility +=
                    fact_mobility / sum_fact_mobility[var][val];
            }
        }
    }
...
bool MWISFactoring::fulfills_min_flexibility_and_mobility(
    const PotentialLeaf& pleaf, const vector<size_t>& included_as,
    const vector<vector<unordered_map<size_t, int>>>& facts_to_mobility,
    ...
    if (min_flexibility > 0.0 || min_mobility > 1) {
        ...
        if (min_flexibility > 0.0) {
            ...
            if (min_flexibility >
                num_leaf_only_actions / pleaf.num_affecting_actions) {
                return false;
            }
        }
        if (min_mobility > 1) {
    ...
    if (min_fact_flexibility > 0) {
        assert(facts_to_mobility.size() ==
               static_cast<size_t>(task->get_num_variables()));
        assert(facts_to_mobility.size() == sum_fact_mobility.size());

        double sum_fact_flexibility = compute_leaf_fact_flexibility(
...
void MWISFactoring::multiply_out_potential_leaf(
    const vector<pair<vector<int>, vector<size_t>>>&
        outside_pre_and_ases,
    const PotentialLeaf& pleaf, vector<int>& outside_pre_vars,
    vector<size_t>& included_as, size_t depth,
    ...
    if (depth == outside_pre_and_ases.size()) {
        ...
        if (fulfills_min_flexibility_and_mobility(pleaf, included_as,
                                                  facts_to_mobility,
                                                  sum_fact_mobility)) {
            potential_leaf_nodes.emplace_back(outside_pre_vars,
                                              pleaf.vars, weight);
        } else {
    ...
    assert(all_of(ases.begin(), ases.end(), [&](size_t as) {
        return find(pleaf.self_mobile_as.begin(),
                    pleaf.self_mobile_as.end(),
                    as) == pleaf.self_mobile_as.end();
    }));
    size_t num_outside_pre_before = outside_pre_vars.size();
    size_t num_included_as_before = included_as.size();

    // (1) include this AS
    outside_pre_vars.insert(outside_pre_vars.end(), outside_pre.begin(),
                            outside_pre.end());
    included_as.insert(included_as.end(), ases.begin(), ases.end());

    multiply_out_potential_leaf(outside_pre_and_ases, pleaf,
                                outside_pre_vars, included_as,
                                depth + 1, ignored_leaf_candidates,
                                facts_to_mobility, sum_fact_mobility);
...
void MWISFactoring::multiply_out_action_schemas(
    ...
    for (auto& pleaf : potential_leaves) {
        vector<pair<vector<int>, vector<size_t>>> outside_pre_and_as;
        {
            ...
            utils::HashMap<vector<int>, vector<size_t>>
                as_by_outside_pre_vars;
            for (size_t as_id : pleaf.action_schemes) {
                if (find(pleaf.self_mobile_as.begin(),
                         pleaf.self_mobile_as.end(),
                         as_id) == pleaf.self_mobile_as.end()) {
                    vector<bool> is_outside_pre_var(
                        task->get_num_variables(), false);
                    ...
                    for (int var = 0; var < task->get_num_variables();
                         ++var) {
                        if (is_outside_pre_var[var]) {
                            outside_pre_vars.push_back(var);
                        }
                    }
                    as_by_outside_pre_vars[outside_pre_vars].push_back(
                        as_id);
                }
            }

            outside_pre_and_as.reserve(as_by_outside_pre_vars.size());
            for (const auto& [outside_pre, ases] :
                 as_by_outside_pre_vars) {
            ...
            if (min_flexibility > 0) {
                ...
                assert(pleaf.num_actions > 0);
                assert(pleaf.num_affecting_actions > 0);
                assert(pleaf.num_affecting_actions >=
                       pleaf.num_actions);
                if (min_flexibility >
                    pleaf.num_actions / (double)num_ops) {
                    // this is the maximum this leaf can possibly
                    // get and it is not enough
                    continue;
                }
            }

            if (min_mobility > pleaf.num_actions) {
                // this is the maximum this leaf can possibly get
                // and it is not enough
                continue;
            }
            if (min_fact_flexibility > 0) {
                double max_leaf_fact_flex =
                    compute_leaf_fact_flexibility(
                        pleaf.vars, *task, pleaf.action_schemes,
                        facts_to_mobility, sum_fact_mobility);
        ...
        // NOTE: it could make sense to filter duplicate leaf node
        // candidates here. even with the grouping of outside
        // precondition vars to action schemas, it can happen that
        // identical candidates are generated. It seems like it only
        // happens in very few instances (2/2000), and even in these
        // instances, only very few duplicates were generated.
        // Hence, we do not prune duplicates to avoid the overhead
        // of, e.g. sort_unique, sorting.

        vector<int> outside_pre_vars;
        vector<size_t> included_as(pleaf.self_mobile_as);
        multiply_out_potential_leaf(
            outside_pre_and_as, pleaf, outside_pre_vars, included_as, 0,
            ignored_leaf_candidates, facts_to_mobility,
            sum_fact_mobility);
    ...
    log << "Ignored " << ignored_leaf_candidates
        << " potential leaves due to minimum mobility / "
           "flexibility."
        << endl;
    log << "Number final leaf candidates: "
        << potential_leaf_nodes.size() << endl;
}

vector<size_t> MWISFactoring::get_superset_pleaf_ids(
    ...
    if (pleaf.vars.size() == 1) {
        ...
        // since we consider all supersets of p_leaf, we can simply
        // take p_leaf.vars[0], since all supersets must also
        // include that variable
        for (size_t check_pleaf_id : var_to_p_leaves[pleaf.vars[0]]) {
            bool superset_schema = true;
            for (int var : pleaf.vars) {
                if (!binary_search(
                        potential_leaves[check_pleaf_id].vars.begin(),
                        potential_leaves[check_pleaf_id].vars.end(),
                        var)) {
...
void MWISFactoring::compute_fact_flexibility(
    ...
    // need to store information for effect *facts*, which gets
    // otherwise lost
    ...
    utils::HashMap<vector<int>, utils::HashMap<vector<int>, size_t>>
        scheme_lookup;
    for (size_t as_id = 0; as_id < action_schemas.size(); ++as_id) {
        const ActionSchema& as = action_schemas[as_id];
    ...
    for (OperatorProxy op : task_proxy.get_operators()) {
        ...
        assert(scheme_lookup.count(pre_vars) > 0 &&
               scheme_lookup[pre_vars].count(eff_vars) > 0);
        size_t as = scheme_lookup[pre_vars][eff_vars];
        for (EffectProxy eff : op.get_effects()) {
            facts_to_mobility[eff.get_fact().get_variable().get_id()]
                             [eff.get_fact().get_value()][as]++;
        }
    }
    ...
    for (int var = 0; var < task->get_num_variables(); ++var) {
        ...
        for (int val = 0; val < variables[var].get_domain_size();
             ++val) {
...
void MWISFactoring::compute_potential_leaves() {
...

    {
        ...
        for (size_t as = 0; as < action_schemas.size(); ++as) {
            ...
            if (it == leaf_lookup.end()) {
                ...
                potential_leaves[it->second].add_leaf_only_schema(
                    as, action_schema);
            }
        }
    }
    ...
    for (const PotentialLeaf& p_leaf : potential_leaves) {
        vector<size_t> superset_schemes(get_superset_pleaf_ids(
            p_leaf, potential_leaves, var_to_p_leaves));
        for (size_t superset_as_id : superset_schemes) {
            for (size_t leaf_only_schema : p_leaf.action_schemes) {
                potential_leaves[superset_as_id].add_leaf_only_schema(
                    leaf_only_schema, action_schemas[leaf_only_schema]);
            }
        }
        if (!check_timeout()) {
            return;
    ...
    multiply_out_action_schemas(potential_leaves, facts_to_mobility,
                                sum_fact_mobility);
...
void MWISFactoring::add_cg_sccs(
    vector<PotentialLeaf>& potential_leaves,
    vector<vector<size_t>>& var_to_p_leaves) {
    ...
    if (sccs.size() == 1) {
        ...
        log << "Causal-graph is strongly connected, no potential "
    ...
    for (auto& scc : sccs) {
        ...
        if (leaf_lookup.find(scc) == leaf_lookup.end()) {
            ...
            for (int scc_var : scc) {
                for (size_t index : var_to_p_leaves[scc_var]) {
                    if (index >= potential_leaves.size() - added) {
                        // potential leaves from other SCCs cannot
                        // be subset
                        continue;
                    }
                    ...
                    for (int var : potential_leaves[index].vars) {
                        if (!binary_search(scc.begin(), scc.end(),
                                           var)) {
                            subset_schema = false;
                            break;
                        }
            ...
            if (subset_schemes.empty()) {
                // this can happen when an SCC is not affected by
                // any action
                continue;
            }
            ...
            for (size_t pleaf : subset_schemes) {
                ...
                for (size_t as_id :
                     potential_leaves[pleaf].action_schemes) {
                    potential_leaves[s].add_leaf_only_schema(
                        as_id, action_schemas[as_id]);
                }
            }
        }
        if (!check_timeout()) {
    ...
    log << "Added " << added << " causal-graph SCC potential leaves."
        << endl;
}
...
void MWISFactoring::add_options_to_parser(plugins::Feature& feature) {
    feature.add_option<WMIS_STRATEGY>(
        ...
        "is being "
        "optimized by the LP, e.g. the number of mobile leaves, or "
        "the sum"
        "of leaf mobility.",
        "MML");
    feature.add_option<int>(
        "min_mobility",
        "Minimum number of leaf-only actions per leaf factor.", "1");
    feature.add_option<double>("min_flexibility",
                               "Minimum flexibility (ratio between "
                               "the number of leaf-only vs. "
                               "all actions affecting a leaf.",
                               "0.2");
    feature.add_option<double>(
        "min_fact_flexibility",
        "Fact flexibility is measured as the mean ratio across all "
        "facts in a leaf of the number of"
        "leaf-only vs. all actions affecting that leaf. This "
        "option "
        "imposes a minimum on that metric.",
        "0");
...
static plugins::TypedEnumPlugin<WMIS_STRATEGY> _enum_plugin({
    ...
    {"MCM", "maximize conclusive mobility, i.e. number of "
            "conclusive actions (sum)"},
});

class MWISFactoringFeature
    : public plugins::TypedFeature<Factoring, MWISFactoring> {
 public:
